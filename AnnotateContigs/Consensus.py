...
class ConsensusSeq:
    def __init__(self, records, threshold):
        ...
        self.seq = []
        weights = [sum([1 for nt in r if nt != '-']) for r in records]
        weights = [w*log(w, 4) for w in weights]
        for tup in zip(*records):
            ...
            for i, nt in enumerate(tup):
                if nt == 'A':
                    ...
                    temp.G += 0.5 * weights[i]
                    temp.T += 0.5 * weights[i]
                elif nt == 'M':
                    temp.A += 0.5 * weights[i]
                    temp.C += 0.5 * weights[i]
                elif nt == 'R':
                    temp.A += 0.5 * weights[i]
                    temp.G += 0.5 * weights[i]
                elif nt == 'Y':
                    ...
                    temp.C += float(1) / 3 * weights[i]
                    temp.G += float(1) / 3 * weights[i]
                    temp.T += float(1) / 3 * weights[i]
                elif nt == 'D':
                    temp.A += float(1) / 3 * weights[i]
                    temp.G += float(1) / 3 * weights[i]
                    temp.T += float(1) / 3 * weights[i]
                elif nt == 'H':
                    temp.A += float(1) / 3 * weights[i]
                    ...
                    temp.A += 0.25 * weights[i]
                    temp.C += 0.25 * weights[i]
                    temp.G += 0.25 * weights[i]
                    temp.T += 0.25 * weights[i]
                else:
                    pass
    ...
    def GetConsensus(self):
        ...
        return [n.GetConNT() for n in self.seq]
